...
import { projectEvents } from './TimelineService.js';
...
export class EventService {
  ...
  async updateEvent(id, updates) {
    ...
    if (isLoan) {
      ...
      if (updateScope === 'subsequent' || propagateForward) {
        await eventRepository.updateMany(
          (ev) =>
            (ev.timelineOriginId === loanTlId || ev.category === 'parcela_emprestimo') &&
    ...
    if (updateScope === 'single' && targetSeriesId) {
      ...
      const existingOverride = allRawEvents.find((ev) => ev.sobrepositionOver === targetSeriesId && ev.date === directUpdates.date);
      if (existingOverride) {
        return eventRepository.update(existingOverride.id, { ...directUpdates, version: nextVersion, sobrepositionOver: targetSeriesId, isRecurring: false, periodicity: 'unico' });
      } else {
        return eventRepository.create({ ...directUpdates, sobrepositionOver: targetSeriesId, version: nextVersion, isRecurring: false, periodicity: 'unico' });
      }
    }

    // Cenário 2: Toda a Série
    if (updateScope === 'all' && targetSeriesId) {
      await eventRepository.updateMany((ev) => ev.seriesId === targetSeriesId || ev.sobrepositionOver === targetSeriesId, directUpdates);
      return true;
    }

    // Cenário 3: Subsequentes
    if ((updateScope === 'subsequent' || propagateForward) && targetSeriesId) {
      ...
      await eventRepository.updateMany(
        ...
        { targetAmount: directUpdates.targetAmount, initialInvestedAmount: directUpdates.initialInvestedAmount, title: directUpdates.title, category: directUpdates.category }
      );

      return eventRepository.create({ ...directUpdates, seriesId: targetSeriesId, version: nextVersion, isRecurring: true, periodicity: 'recorrente' });
    }

    if (existing) return eventRepository.update(id, directUpdates);
    if (targetSeriesId) return eventRepository.create({ ...directUpdates, sobrepositionOver: targetSeriesId, isRecurring: false });
  ...
  async toggleEventPayment(id) {
    ...
    if (event) {
      if (event.category === 'amortizacao' || event.financialType === 'amortizacao' || event.isAmortization) {
        ...
        const updated = await eventRepository.update(id, { status: newStatus, isCompleted });
        if (isCompleted) await this.processLoanAmortization({ ...event, status: 'Amortizado', isCompleted: true });
        else await this.rollbackLoanAmortization(event);
        return updated;
      }

      const isInvestment = event.isInvestment || event.financialType === 'investimento' || event.category?.startsWith('investimento');
      const isIncome = event.isIncome || event.financialType === 'entrada' || event.category?.startsWith('entrada');
      ...
      return eventRepository.update(id, { status: newStatus, isCompleted });
    }
    ...
    if (projEv && projEv.seriesId) {
      const isInvestment = projEv.isInvestment || projEv.financialType === 'investimento' || projEv.category?.startsWith('investimento');
      const isIncome = projEv.isIncome || projEv.financialType === 'entrada' || projEv.category?.startsWith('entrada');
      ...
      return eventRepository.create({ ...projEv, sobrepositionOver: projEv.seriesId, version: currentHighestVersion + 1, status: newStatus, isCompleted, isRecurring: false });
    }
  ...
  async deleteEvent(id, options = {}) {
    ...
    const isLoan = directEvent?.category === 'parcela_emprestimo' || directEvent?.isSystemLoanEvent || directEvent?.timelineOriginId?.startsWith('tl-loan-');
    ...
    if (deleteScope === 'all' || options.deleteSeries) {
      if (targetSeriesId) {
        await eventRepository.deleteMany((ev) => ev.seriesId === targetSeriesId || ev.sobrepositionOver === targetSeriesId || ev.id === id);
        return true;
      }
      return eventRepository.delete(id);
    }
    ...
    if (deleteScope === 'subsequent' && targetSeriesId) {
      ...
      await eventRepository.create({
        ...(directEvent || {}), seriesId: targetSeriesId, version: currentHighestVersion + 1, date: targetDate,
        isTerminated: true, isDeleted: true, isRecurring: true, periodicity: 'recorrente',
    ...
    if (targetSeriesId) {
      ...
      const existingOverride = allRawEvents.find((ev) => ev.sobrepositionOver === targetSeriesId && ev.date === targetDate);
      if (existingOverride) {
        return eventRepository.update(existingOverride.id, { isDeleted: true, status: 'Excluido', version: nextVersion });
      }
      await eventRepository.create({
        seriesId: targetSeriesId, sobrepositionOver: targetSeriesId, date: targetDate, version: nextVersion,
        isDeleted: true, status: 'Excluido',
        title: directEvent?.title ? `${directEvent.title} (Excluído)` : 'Ocorrência Excluída',
        isRecurring: false, periodicity: 'unico'
      });
      return true;
    }
  ...
  async processLoanAmortization(amortEvent) {
    ...
    if (strategy === 'reduce_term') {
      ...
      for (let i = futureUnpaid.length - 1; i >= 0; i--) {
        ...
        if (remainingToDeduct >= instPrincipal) {
          updatesMap.set(inst.id, {
            status: 'Abatida', isAbatida: true, isCompleted: true,
            originalAmount: inst.amount || instPrincipal, amount: 0, principalAmount: 0, interestPortion: 0,
            labels: Array.from(new Set([...(inst.labels || []), 'Abatida'])), updatedAt: now
          });
          ...
          updatesMap.set(inst.id, {
            amount: Math.round((newPrincipal + interestPortion) * 100) / 100, principalAmount: newPrincipal,
            labels: Array.from(new Set([...(inst.labels || []), 'Abatida Parcial'])), updatedAt: now
          });
          remainingToDeduct = 0;
        }
      }
      ...
      const futureUnpaid = loanInstallments.filter((ev) => ev.status !== 'Pago' && ev.status !== 'Abatida' && !ev.isAbatida && ev.date >= amortDate);
      if (futureUnpaid.length > 0) {
        ...
        const updatedAll = rawEvents.map((ev) => {
          if (futureIds.has(ev.id)) {
            ...
            return { ...ev, originalAmount: origAmt, amount: newTotal, principalAmount: Math.round(origCap * reductionRatio * 100) / 100, interestPortion: Math.round(origJur * reductionRatio * 100) / 100, updatedAt: now };
          }
          return ev;
        });
        await eventRepository._writeAllRaw(updatedAll);
    ...
    // Atualizar dívida no loan_contracts e timelines
    try {
      const loan = await loanContractRepository.getById(loanTlId);
      if (loan) {
        await loanContractRepository.update(loan.id, {
          remainingDebt: Math.max(0, Math.round((Number(loan.remainingDebt || loan.totalDebt) - amortVal) * 100) / 100),
          amortizedCapital: Math.round(((Number(loan.amortizedCapital) || 0) + amortVal) * 100) / 100
        });
      }
  ...
  async rollbackLoanAmortization(amortEvent) {
    ...
    const updatedEvents = rawEvents.map((ev) => {
      if (
        (ev.timelineOriginId === loanTlId || (ev.category === 'parcela_emprestimo' && ev.timelineOriginName === amortEvent.timelineOriginName)) &&
        ev.category === 'parcela_emprestimo' && ev.status !== 'Pago'
      ) {
        let cap = 0, jur = 0;
        if (ev.description) {
        ...
        return {
          ...ev, status: 'Pendente', isAbatida: false, isCompleted: false, amount: origAmt,
...
function hasEventMeaningfullyChanged(base, updates) {
  ...
  if (updates.breakdownItems !== undefined && JSON.stringify(base.breakdownItems || []) !== JSON.stringify(updates.breakdownItems || [])) return true;
  if (updates.labels !== undefined && JSON.stringify([...(base.labels || [])].sort()) !== JSON.stringify([...(updates.labels || [])].sort())) return true;
  return false;
}
