-- Stored Procedure SQL Supabase: get_investment_timeline_metrics.sql
-- Retorna os dados agregados para o InvestmentTimelineHeader (Header de Investimentos e Poupança)

DROP FUNCTION IF EXISTS get_investment_timeline_metrics(UUID, DATE);

CREATE OR REPLACE FUNCTION get_investment_timeline_metrics(
  p_timeline_id UUID,
  p_reference_date DATE DEFAULT CURRENT_DATE
)
RETURNS TABLE (
  current_month_invested NUMERIC,
  current_month_income NUMERIC,
  monthly_target NUMERIC,
  savings_rate_percent NUMERIC,
  committed_amount_30 NUMERIC,
  committed_count_30 INT,
  paid_amount_month NUMERIC,
  paid_count_month INT,
  diff_previous_month_percent NUMERIC,
  is_diff_positive BOOLEAN,
  annual_projected_invested NUMERIC,
...
DECLARE
  ...
  v_curr_inv NUMERIC := 0;
  v_curr_inc NUMERIC := 0;
  v_prev_inv NUMERIC := 0;
  v_comm_amt_30 NUMERIC := 0;
  v_comm_cnt_30 INT := 0;
  v_paid_amt_month NUMERIC := 0;
  v_paid_cnt_month INT := 0;
...
BEGIN
  ...
  -- 1. Total Investido / Aportado no Mês Atual (Estritamente event_type = 'investment')
  SELECT 
  ...
  -- Total de Entradas do Mês (para cálculo de taxa de poupança se aplicável)
  SELECT 
    COALESCE(SUM(e.amount), 0)
  INTO v_curr_inc
  FROM financial_events e
  ...
  -- 2. Total Investido no Mês Anterior
  SELECT 
    COALESCE(SUM(e.amount), 0)
  INTO v_prev_inv
  FROM financial_events e
  ...
  -- 3. Comprometido nos Próximos 30 Dias & Já Aportados/Liquidados no Mês
  SELECT 
  ...
  INTO v_comm_amt_30, v_comm_cnt_30, v_paid_amt_month, v_paid_cnt_month
  FROM financial_events e
  LEFT JOIN financial_event_status fs 
    ON fs.event_id = e.event_id 
   AND fs.year = EXTRACT(YEAR FROM e.date)::INT 
  ...
  -- 4. Breakdown por Categoria (Lê estritamente e.category nos valores do InvestmentEventCategory enum)
  WITH cat_totals AS (
    SELECT 
      CASE 
  ...
  -- 5. Histórico de Volume Mensal de Aportes (Últimos 6 Meses + Mês Atual)
  ...
  IF v_prev_inv > 0 THEN
    v_diff_pct := ROUND(((v_curr_inv - v_prev_inv) / v_prev_inv) * 100, 1);
    v_diff_pos := v_diff_pct >= 0;
  ELSIF v_curr_inv > 0 THEN
    v_diff_pct := 100.0;
    v_diff_pos := TRUE;
  ELSE
    v_diff_pct := 0.0;
  ...
  v_ann_proj := ROUND(v_curr_inv * 12, 2);

  RETURN QUERY SELECT 
    v_curr_inv,
    v_curr_inc,
    500.00::NUMERIC, -- Meta de aporte mensal padrão
    CASE WHEN v_curr_inc > 0 THEN ROUND((v_curr_inv / v_curr_inc) * 100, 1) ELSE 0 END,
    v_comm_amt_30,
    v_comm_cnt_30,
    v_paid_amt_month,
    v_paid_cnt_month,
